...
(defrecord InProcess [adapter conn crashes]
  target/Connection
  (acquire! [this]
    ;; Workers come and go -- and a crashed process opens a fresh client -- but
    ;; they all want the instance that's live now, not one of their own.
    (swap! conn #(or % (client/open adapter)))
    this)
...
(defmethod target/build :in-process [_target adapter]
  (map->InProcess {:adapter adapter, :conn (atom nil), :crashes (atom 0)}))
